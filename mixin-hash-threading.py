...
def saveDataTxVinMixinHash(height, tx_idx, tx_hash, vin_idx, k_image, vout_key_hash, vout_key_count):
    ...
    s = s % (height, tx_idx, QuotedStr(tx_hash), vin_idx, QuotedStr(k_image), QuotedStr(vout_key_hash), vout_key_count)
    #print s
    cnx = mysql.connector.connect(**mysqlconfig)
    cur = cnx.cursor()
    try:
...
def hashHeight(height):
    ...
    for row in rows:
        ...
        vin_idx = row[2]
        k_image = row[3]
        vout_key_hash = hashData(mixinOfKimageStr(k_image))
        vout_key_count = mixinOfKimageCount(k_image)
        
        #insert data
        saveDataTxVinMixinHash(height, tx_idx, tx_hash, vin_idx, k_image, vout_key_hash, vout_key_count)
...
def runHashHeight(threadSize):
    ...
    firstHeight = dbGetLastProcessedHeight()-100
    #firstHeight = 110
