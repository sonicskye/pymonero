...
def saveBlockDataToDB(threadSize):
    ...
    while (dbLastHeight <= (moneroLastHeight - 10)):
        ...
        for i in range(1,threadSize):
            if dbLastHeight < moneroLastHeight:
                # Create new threads
                thread = myThreadHeader(i, "Thread-"+str(i), nextHeight)
                # Start new Threads
                thread.start()
...
def fixMissingHeader(threadSize):
    ...
    cnx = mysql.connector.connect(**mysqlconfig)
    cur = cnx.cursor()
    #s = "select count(h.height) from height_test h where h.HEIGHT not in (select t.height from header t) and h.height < 714500"
    #s = "select count(distinct m.header_height) from tx_vin_mixin m where not exists (select h.tx_hash from header_tx h where h.tx_hash = m.tx_hash);"
    #tx_vout based
    s = "select count(distinct m.header_height) from tx_vin_mixin m where not exists (select v.tx_hash from tx_vout v where v.tx_hash = m.tx_hash and v.vout_key = m.vout_key);"
    #tx_vin based
    #s = "select count(distinct m.header_height) from tx_vin_mixin m where not exists (select v.tx_hash from tx_vin v where v.tx_hash = m.tx_hash and v.k_image = m.k_image);"
    ...
    cnx = mysql.connector.connect(**mysqlconfig)
    cur = cnx.cursor()
    #s = "select h.height from height_test h where h.HEIGHT not in (select t.height from header t) and h.height < 714500"
    #s = "select distinct m.header_height from tx_vin_mixin m where not exists (select h.tx_hash from header_tx h where h.tx_hash = m.tx_hash);"
    #tx_vout based
    s = "select distinct m.header_height from tx_vin_mixin m where not exists (select v.tx_hash from tx_vout v where v.tx_hash = m.tx_hash and v.vout_key = m.vout_key);"
    #tx_vin based
    #s = "select distinct m.header_height from tx_vin_mixin m where not exists (select v.tx_hash from tx_vin v where v.tx_hash = m.tx_hash and v.k_image = m.k_image);"
